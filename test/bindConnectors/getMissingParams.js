...
describe('#getMissingParams', function () {
	...
	it('should pass when a pure input schema is passed in, and conditions pass', function () {
		var missingParams = getMissingParams(
			{
				test1: 'test1',
				test2: 123,
	...
	it('should fail when missing properties in a pure input schema', function () {
		var missingParams = getMissingParams(
			{
				test3: [{}],
				test4: {
		...
		assert.equal(missingParams[0], 'test2'); // missing local
		assert.equal(missingParams[1], 'test1'); // missing global
	});
