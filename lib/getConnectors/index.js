	...
	connectors = _.map(connectorDirectories, function (connectorDirectory) {
		...
		function requireFile (path, errorMessageFile) {
			try {
				...
				throw new Error('Error with ' + connector.name + errorMessageFile + ':\n' + err.message);
			}
		}

		// Add the global model file
		if (result.indexOf('global_model.js') !== -1) connector.globalModel = requireFile(fullConnectorPath + '/global_model', '/global_model.js');		// deprecated file name
		else if (result.indexOf('global.js') !== -1) connector.globalModel = requireFile(fullConnectorPath + '/global', '/global_model.js');
		...
		connector.messages = _.map(
			...
			function (message) {
				...
				if (files.indexOf('dynamic_output.js') !== -1) {
					output['dynamicOutput'] = requireFile(messageDir + '/dynamic_output', 'dynamic_output.js');
				}
