...
module.exports = function (config, options, dependenciesLoaded) {
	// console.log(util.inspect(config, false, null));

	var messageHooks = bindMessageHooks(config, options, dependenciesLoaded);
		...
		var promises = _.map(events, function (event) {
			...
			if (promiseTimeout) {
				...
				.catch(when.TimeoutError, function () {
					...
					return when.resolve(formatError(
						{
							headers: {},
							body: {
								code: '#timeout_error',
