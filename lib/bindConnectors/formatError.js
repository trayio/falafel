...
var isAPIResponseError = require('../utils/isAPIResponseError.js');
...
module.exports = function (response, event) {
	...
	/*
		Classify the failure before the branching below: the `_.isError` branch
		rebuilds `details` from scratch, which would otherwise discard this.
	*/
	var isFailedAPIResponse = isAPIResponseError(details);
	...
	/*
		Whether the failure was an API response that violated the connector's
		`expects`/`notExpects` configuration, as opposed to a hook throw, a
		transport failure or a connector bug.
	*/
	details.isAPIResponseError = isFailedAPIResponse;
