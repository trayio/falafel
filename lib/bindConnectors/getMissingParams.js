...
module.exports = function (params, messageSchema, globalSchema) {
	...
	// IMPORTANT: `globalSchema` will be a blank object for newer (Artisan) connectors,
	// so it's functionality is now deprecated.
		...
		_.each(globalSchema.input || {}, function (schemaObj, key) {
			if (schemaObj.required === true) {
				requiredKeys.push({
					key: key,
					type: schemaObj.type,
					emptyRequired: schemaObj.emptyRequired
				});
			}
		});
	...
	return _.reduce(requiredKeys, function (missingParams, reqObj) {
		var missingCheck = (
			...
			( params[reqObj.key] === '' && reqObj.emptyRequired !== true ) ||
