...
var validateInputTypes = require('./validateInputTypes');
...
module.exports = function (message, threadneedle, connectorConfig) {
	...
	return function (event) {
		return when.promise(function (resolve, reject) {
			...
			// Validate input types (skip for DDL operations starting with #)
			var typeErrors = (
				methodName[0] === '#' ?
					[] :
					validateInputTypes.validateInputTypes(event.body, message.schema, connectorConfig.globalSchema)
			);
			...
			if (typeErrors.length) {
				return reject({
					...
					body: {
						code: '#user_input_error',
						message: 'Input validation errors: ' + typeErrors.join('; ')
					}
				});
			}
