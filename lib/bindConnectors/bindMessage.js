	...
	return function (event) {
		return when.promise(function (resolve, reject) {
			...
			if (missingParams.length) {
				...
				// var inputBody = formatInputBody(event.body, message.schema);
				//
				// threadneedle[methodName](inputBody)
				//
				...
				// 	};
				//
				// 	if (message.model.hook && _.isFunction(message.model.hook)) {
				// 		response.header = message.model.hook(inputBody, result);
				// 	}
				// 	return response;
				// })
				//
