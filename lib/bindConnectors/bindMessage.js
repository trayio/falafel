...
module.exports = function (message, threadneedle, connector, connectorConfig) {
	...
	if (connectorConfig.globalSchema) {
		_.each(connectorConfig.globalSchema.input || {}, function (obj, key) {
			if (obj.required === true) {
				requiredKeys.push(key);
			}
		});
