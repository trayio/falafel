...
module.exports = function (config) {
	...
	_.each(config.operations, function (operation) {
		...
		var globalProperties = _.omit(operation.globalSchema.properties, operation.excludeGlobalProperties);
		...
		_.each(operation.globalSchema.required, (requiredKey) => {
			var notExcluded = !_.includes(operation.excludeGlobalProperties, requiredKey);
			var notInRequired = !_.includes(message.schema.required, requiredKey);
			if (notExcluded && notInRequired) {
