...
var getRequestModel 	= require('./getRequestModel');
var getResponseModel 	= require('./getResponseModel');
	...
	_.each(config.operations, function (operation) {
		...
		if (_.isObject(operation.subOperations)) {	
			...
			if (_.isObject(subOperations.request)) {
				message.request = getRequestModel(subOperations.request);
			}
			if (_.isObject(subOperations.response)) {
				message.response = getResponseModel(subOperations.response);
			}
		}
