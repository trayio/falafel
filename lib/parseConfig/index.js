...
module.exports = function (config) {
	...
	_.each(config.operations, function (operation) {
		...
		// Sub operations: if the operation has any, add those too.
		if (_.isObject(operation.subOperations)) {
			_.each(operation.subOperations, function (subOperation, subOperationName) {
				var subMessage = {
					name: operation.name + '_' + subOperationName
				};
				subMessage.model = getModel(subOperation);
				connector.messages.push(subMessage);
			});
		}
