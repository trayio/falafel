...
var getDynamicOutput	= require('./getDynamicOutput');
var getHelper = require('./getHelper');
...
module.exports = function (config) {
	...
	_.each(config.operations, function (operation) {
		...
		var dynamicOutput = getDynamicOutput(operation);
		if (dynamicOutput) {
			message.dynamicOutput = dynamicOutput;
		}
	...
	// Set the helpers
	connector.helpers = {};
	_.forEach(config.helpers, function (parameter, key) {
		if (!trim(key)) {
			return;
		}
		connector.helpers[key] = getHelper(parameter, key);
	});
