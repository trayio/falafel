...
function generateErrorMessage (whitelistedBaseUrls) {
	...
	if (whitelistBases.length && whitelistRegexes.length) {
		errorMessage += `\nThe URL must either \n - start with: ${whitelistBases.join(' or ')} \n - match the following regex(es):  ${whitelistBases.join(' or ')}`;
	} else if (whitelistBases.length) {
		errorMessage += `\nThe URL must start with: ${whitelistBases.join(' or ')}.`;
	} else if (whitelistRegexes.length) {
		errorMessage += `\nThe URL must match the following regex(es): ${whitelistBases.join(' or ')}.`;
	}
