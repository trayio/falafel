...
module.exports = function (error, params, res) {
	...
	/*
		Only augment a genuine API response error, and only when threadneedle
		actually gave us a `response` object to add to - a reshaped error
		identified by its code alone has nowhere sensible to put the headers.
	*/
	if (isAPIResponseError(error) && _.isPlainObject(error.response)) {
