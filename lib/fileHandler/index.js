...
module.exports = function (options) {
	...
	*/
	...
	function checkExpiryConditions (url, headers, rawBody) {
		return (
			( _.includes(url, 'Expires=') || _.includes(url, 'X-Amz-Date=') )&&
			_.includes(headers['content-type'], 'application/xml') &&
			_.includes(headers.server, 'AmazonS3') &&
			_.includes(rawBody.toString(), 'Request has expired')
		);
	}

	function expiryRejectObject (url, headers) {
		const payload = {};
		const targetUrl = new URL(url);

		if (targetUrl.searchParams.has('Expires')) {
			const expiryTime = targetUrl.searchParams.get('Expires');
			if (expiryTime) {
				//snake_case as this is exposed to the user
				payload.datetime_expired = moment(targetUrl.searchParams.get('Expires'), 'X').format();
			}
		}

		if (targetUrl.searchParams.has('X-Amz-Date')) {
			const expiryDate = targetUrl.searchParams.get('X-Amz-Date');
			if (expiryDate) {
				//snake_case as this is exposed to the user
				payload.datetime_expired = moment(targetUrl.searchParams.get('X-Amz-Date'), 'YYYYMMDD[T]HHmmss[Z]').format();
			}
		}
		...
		return when.promise(function (resolve, reject) {
			...
			needle.get(
				...
				function (err, res) {
					...
					} else if (_.inRange(res.statusCode, 400, 500) && checkExpiryConditions(file.url, res.headers, res.raw)) {
						reject(expiryRejectObject(file.url, res.headers));
					} else {
		...
		return when.promise(function (resolve, reject) {
			...
			.on('header', async function (statusCode, headers) {
				...
				if (_.inRange(statusCode, 200, 300)) {
					...
					try {
						await new Promise((expiredResolve, expiredReject) => {
							...
							function dataConsumer (chunk) {
								acc += chunk.toString();
								if (checkExpiryConditions(file.url, headers, acc)) {
									expiredResolve();
						...
						//If it gets to here, checkExpiryConditions is true
						reject(expiryRejectObject(file.url, headers));
					} catch (err) {
