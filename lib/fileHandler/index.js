	...
	*/
	...
	function expiryRejectObject (url, headers) {
		...
		if (targetUrl.searchParams.has('Expires')) {
			...
			if (expiryTime) {
				payload.datetimeExpired = moment(targetUrl.searchParams.get('Expires'), 'X').format();
			}
		}
		...
		if (targetUrl.searchParams.has('X-Amz-Date')) {
			...
			if (expiryDate) {
				payload.datetimeExpired = moment(targetUrl.searchParams.get('X-Amz-Date'), 'YYYYMMDD[T]HHmmss[Z]').format();
			}
		}
