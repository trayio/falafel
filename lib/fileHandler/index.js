...
const REQUIRED_FILE_PROPS = [
	...
	{
		key: 'mime_type',
		/*	TODO: this allows for `null` to make falafel more backwards compatible
			friendly when recevining inputs from other connectors. In v2, this
			should only allow string	*/
		validator: (mimeType) => { return _.isString(mimeType) || _.isNull(mimeType); }
	},
	{
		key: 'expires',
...
function resolveMimeType (params) {
	...
	return _.isNull(mimeType) ? 'application/octet-stream' : mimeType;
}

function resolveMimeTypeWithHeaders (mimeType, headers) {
	if (_.isNull(mimeType)) {
		if (_.isString(headers['content-type']) && headers['content-type'].length) {
	...
	*/
	const upload = function (params) {
		...
		return when.promise(function (resolve, reject) {
			...
			//Get mimeType
			const mimeType = resolveMimeType(params);
				...
				function (err, res) {
					...
					if (res.statusCode === 200) {
						...
						return resolve({
							...
							name: file.name,
							mime_type: resolveMimeTypeWithHeaders(file.mime_type, res.headers),
			...
			.on('header', async function (statusCode, headers) {
				...
				if (_.inRange(statusCode, 200, 300)) {
					resolve({
						...
						// Pass back what was passed in to be helpful
						name: file.name,
						mime_type: resolveMimeTypeWithHeaders(file.mime_type, headers),
						expires: file.expires,
