...
const checkIfStr = (val) => { return typeof val === 'string'; };

const executeTypeValidation = (key, value) => {
	const fieldValidationMap = {
		name: (value) => { return checkIfStr(value); },
		url: (value) => { return checkIfStr(value); },
		mime_type: (value) => { return checkIfStr(value); },
		expires: (value) => { return typeof value === 'number'; }
	};

	const isValidType = fieldValidationMap[key](value);
...
const runFileValidation = (fileObj) => {
	...
	const isValid =  REQUIRED_FILE_PROPS.every((reqField) => { return fileObjKeyArr.includes(reqField); });
	if (isValid) {
		const invalidTypes = REQUIRED_FILE_PROPS.reduce((acc, key) => {
			const isKeyInvalid = executeTypeValidation(key, fileObj[key]);
			if (isKeyInvalid) {
				acc.push(key);
				return acc;
			}
...
const throwFileValidationErr = () => {
	return when.reject({
		...
		message: `The file object passed through must contain all properties 'name', 'url', 'mime_type', 'expires'.`
	});
};

const throwTypeValidationErr = (typeArr) => {
	return when.reject({
		code: '#connector_error',
		message: `The file object passed through contains invalid types for the following properties: ${typeArr.join(', ')}.`
	});
}; 
...
module.exports = function (options) {
	...
	*/
	const download = function (file, needleOptions = {}) {
		...
		// If boolean, then file prop missing. 
		if (!validationResult) {
			return throwFileValidationErr();
		}

		// If array, then incorrect type.
		if (Array.isArray(validationResult)) {
			return throwTypeValidationErr(validationResult);
		}
